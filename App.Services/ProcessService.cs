...
public partial class ProcessService : IProcessService
{
    ...
    private static FileSettings _fileSettings = null!;
    private static SearchSettings _searchSettings = null!;

    [GeneratedRegex(@"(?<ResultCount>\d+)")] private static partial Regex ResultCountRegex();
    //[GeneratedRegex(@"(?<Location>.+)[\d+] miles away")] private static partial Regex NoLocationRegex();
    //[GeneratedRegex(@"(?<Price>£\d+,\d+)")] private static partial Regex PriceRegex();
    //[GeneratedRegex(@"(?<Mileage>\d+,\d+)")] private static partial Regex MileageRegex();

    private static readonly string AssetsDir = Path.Combine(Environment.CurrentDirectory, "Assets");
    private static string _runDir = null!;
    ...
    public async Task Process(RunOptions options)
    {
        ...
        try
        {
            foreach (var searchConfig in _searchSettings.Searches)
            {
                ...
                var searchDirectory = SetupSearch(searchConfig);

                // scrape autotrader for this search config
                var results = await Scrape(searchConfig, searchDirectory);
    ...
    /// <returns></returns>
    private static async Task<Results> Scrape(SearchConfig searchConfig, string searchDirectory)
    {
        // build the search URL
        var searchUrl = Url.Combine(_searchSettings.Domain, searchConfig.Url);
        ...
        try
        {
            var acceptCookies = await page.WaitForSelectorAsync(".message-container .message-button:last-child", new PageWaitForSelectorOptions { Timeout = 4000, State = WaitForSelectorState.Attached });
        ...
        await page.WaitForSelectorAsync("h1[data-testid='search-desktop-result-count']", new PageWaitForSelectorOptions { Timeout = 4000, State = WaitForSelectorState.Visible });
        var resultEls = await page.QuerySelectorAsync("h1[data-testid='search-desktop-result-count']");

        var resultsCount = 0;
        var pages = resultEls != null
        ...
        for (var i = 1; i <= pages; i++)
        {
            ...
            // get adverts on page
            var ads = await page.QuerySelectorAllAsync("a[data-testid='search-listing-title']");
            ...
            foreach (var ad in ads)
            {
                ...
                // snapshot entire ad
                const string adSelector = "#layout-desktop";
                    
                if (await ElementExists(adSelector, adPage, "ad image", false))
                {
                    var image = await adPage.QuerySelectorAsync(adSelector);
                ...
                bool locationLinkFound;

                // try to find a location link
                try
                {
                    const string locBtnSelector = "button[data-gui='dealerLocationLink']";
                    ...
                    await adPage.ClickAsync(locBtnSelector, new PageClickOptions { Timeout = 1 });
                    
                    const string mapFrameSelector = "iframe[src^='https://www.google.com/maps/embed']";
                    ...
                    const string coordsSelector = "div.place-name";
                    ...
                    const string addressSelector = "div.address";
    ...
    /// </summary>
    private static async Task SetupRun(RunOptions options)
    {
    ...
    /// </summary>
    private static string SetupSearch(SearchConfig searchConfig)
    {
    ...
    /// <param name="searchDirectory"></param>
    private static async Task BuildMapScript(Results results, string searchDirectory)
    {
